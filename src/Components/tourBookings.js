...
export default class TourBookings extends Component {
    constructor(props) {
    ...
    async receivedData() {
        ...
        axios
            .post(`${GURL.BASEURL}getTourBookings`, {id:user.app_user_id})
            .then(res => {
                ...
                const tour = nd.tour;
                const tourImage = nd.tour_images;
                console.log(nd)
                return (
                    <div className="row" onClick={()=>this.props.history.push(`tour_booking_details/${nd.request_id}`)}>

                        <div className="col mt-3">
                            <div className="notification_card">
                                <div className="notification_card_left">
                                    <img src={`${GURL.SERVER_IMAGES}tourImages/${tour ? tourImage[0]:null}`} alt="logo" />
                                </div>
                                <div className="notification_card_right">
                                    <h6>{tour ? tour.tour_title:null}</h6>
                                    <p className="d-inline-block">{tour ? tour.tour_type:null}</p> <p className="badge d-inline-block" style={{float:'right'}}>{nd.request_status=="Received"? "In Progress" : nd.request_status}</p>
                                    <p>{tour ? nd.start_from:null} - {nd ? nd.end_at:null}</p>
                                </div>
                            </div>
                            <div className="notification_card_details">
                                <ul>
                                    <li>Person: 0{nd.request_person}</li>
                                    <li>Couple: 0{nd.request_couple}</li>
                                </ul>
